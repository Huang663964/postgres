...
$node->safe_psql('postgres', q[CREATE DATABASE dbbranch_set_logged_drain_source;]);
$node->safe_psql(
	'dbbranch_set_logged_drain_source',
	q[
CREATE UNLOGGED TABLE set_logged_rows (id int PRIMARY KEY, note text NOT NULL);
INSERT INTO set_logged_rows VALUES (1, 'unlogged'), (2, 'before-logged');
CHECKPOINT;
]);

my $set_logged_writer =
  $node->background_psql('dbbranch_set_logged_drain_source', on_error_stop => 1);
$set_logged_writer->query_safe(
	q[
BEGIN;
ALTER TABLE set_logged_rows SET LOGGED;
INSERT INTO set_logged_rows VALUES (3, 'after-logged');
]);
...
$result = $node->psql(
	...
	q[CREATE BRANCH dbbranch_set_logged_drain_target FROM DATABASE dbbranch_set_logged_drain_source],
	stderr => \$stderr);
is($result, 0, 'db branch succeeds after source set logged rewrite drains');

@metadata_files = glob $node->data_dir . '/global/pg_dbbranch_*.state';
$metadata_file_count++;
is(scalar @metadata_files, $metadata_file_count,
	'set logged branch writes separate metadata file');

my $set_logged_persistence = $node->safe_psql(
	'dbbranch_set_logged_drain_target',
	q[SELECT relpersistence FROM pg_class WHERE relname = 'set_logged_rows';]);
is($set_logged_persistence, 'p',
	'branch sees logged persistence after source set logged drains');
my $set_logged_rows = $node->safe_psql(
	'dbbranch_set_logged_drain_target',
	q[SELECT string_agg(id || ':' || note, ',' ORDER BY id) FROM set_logged_rows;]);
is($set_logged_rows, '1:unlogged,2:before-logged,3:after-logged',
	'branch reads rows after source set logged drains');

$node->safe_psql('postgres', q[DROP DATABASE dbbranch_set_logged_drain_target;]);
$node->safe_psql('postgres', q[DROP DATABASE dbbranch_set_logged_drain_source;]);
...
$node->safe_psql('postgres', 'CREATE DATABASE dbbranch_stats_lifecycle_source;');
$node->safe_psql(
	'dbbranch_stats_lifecycle_source',
	q[
CREATE TABLE stats_lifecycle_rows (
	id int PRIMARY KEY,
	grp int NOT NULL,
	name text NOT NULL
);
INSERT INTO stats_lifecycle_rows VALUES (1, 1, 'before-stats'), (2, 0, 'kept');
CHECKPOINT;
CREATE STATISTICS stats_lifecycle_id_grp ON id, grp FROM stats_lifecycle_rows;
INSERT INTO stats_lifecycle_rows VALUES (3, 1, 'after-stats');
]);

$stderr = '';
$result = $node->psql(
	'postgres',
	q[CREATE BRANCH dbbranch_stats_lifecycle_target FROM DATABASE dbbranch_stats_lifecycle_source],
	stderr => \$stderr);
is($result, 0, 'db branch supports extended statistics');

my $stats_lifecycle_rows = $node->safe_psql(
	'dbbranch_stats_lifecycle_target',
	q[SELECT string_agg(id || ':' || name, ',' ORDER BY id) FROM stats_lifecycle_rows;]);
is($stats_lifecycle_rows, '1:before-stats,2:kept,3:after-stats',
	'branch reads rows after extended statistics creation');
my $stats_lifecycle_exists = $node->safe_psql(
	'dbbranch_stats_lifecycle_target',
	q[SELECT count(*) FROM pg_statistic_ext WHERE stxname = 'stats_lifecycle_id_grp';]);
is($stats_lifecycle_exists, '1', 'branch exposes extended statistics');
...
$node->safe_psql('postgres', 'CREATE DATABASE dbbranch_drop_base_lifecycle_source;');
$node->safe_psql(
	'dbbranch_drop_base_lifecycle_source',
	q[
CREATE TYPE drop_base_lifecycle_type;
CREATE FUNCTION drop_base_lifecycle_in(cstring, oid, integer)
RETURNS drop_base_lifecycle_type
LANGUAGE internal IMMUTABLE PARALLEL SAFE STRICT AS 'varcharin';
CREATE FUNCTION drop_base_lifecycle_out(drop_base_lifecycle_type)
RETURNS cstring
LANGUAGE internal IMMUTABLE PARALLEL SAFE STRICT AS 'varcharout';
CREATE TYPE drop_base_lifecycle_type (
	input = drop_base_lifecycle_in,
	output = drop_base_lifecycle_out,
	alignment = integer,
	storage = extended
);
CREATE TABLE drop_base_lifecycle_rows (
	id int PRIMARY KEY,
	payload drop_base_lifecycle_type,
	note text NOT NULL
);
INSERT INTO drop_base_lifecycle_rows
VALUES (1, 'before'::drop_base_lifecycle_type, 'before-drop');
CHECKPOINT;
DROP TYPE drop_base_lifecycle_type CASCADE;
INSERT INTO drop_base_lifecycle_rows (id, note)
VALUES (2, 'after-drop');
]);
...
$result = $node->psql(
	'postgres',
	q[CREATE BRANCH dbbranch_drop_base_lifecycle_target FROM DATABASE dbbranch_drop_base_lifecycle_source],
	stderr => \$stderr);
is($result, 0, 'db branch supports dropped base types');

my $drop_base_lifecycle_rows = $node->safe_psql(
	'dbbranch_drop_base_lifecycle_target',
	q[SELECT string_agg(id || ':' || note, ',' ORDER BY id) FROM drop_base_lifecycle_rows;]);
is($drop_base_lifecycle_rows,
	'1:before-drop,2:after-drop',
	'branch reads rows after base type drop');
my $drop_base_lifecycle_columns = $node->safe_psql(
	'dbbranch_drop_base_lifecycle_target',
	q[
SELECT string_agg(column_name, ',' ORDER BY ordinal_position)
FROM information_schema.columns
WHERE table_schema = 'public'
  AND table_name = 'drop_base_lifecycle_rows';
]);
is($drop_base_lifecycle_columns, 'id,note',
	'branch does not expose column from dropped base type');
my $drop_base_lifecycle_type = $node->safe_psql(
	'dbbranch_drop_base_lifecycle_target',
	q[
SELECT count(*)
FROM pg_type t
JOIN pg_namespace n ON n.oid = t.typnamespace
WHERE n.nspname = 'public'
  AND t.typname = 'drop_base_lifecycle_type';
]);
is($drop_base_lifecycle_type, '0',
	'branch does not expose dropped base type');
my $drop_base_lifecycle_functions = $node->safe_psql(
	'dbbranch_drop_base_lifecycle_target',
	q[
SELECT count(*)
FROM pg_proc p
JOIN pg_namespace n ON n.oid = p.pronamespace
WHERE n.nspname = 'public'
  AND p.proname IN ('drop_base_lifecycle_in', 'drop_base_lifecycle_out');
]);
is($drop_base_lifecycle_functions, '0',
	'branch does not expose functions from dropped base type');
...
$node->safe_psql('postgres', 'CREATE DATABASE dbbranch_drop_opfamily_lifecycle_source;');
$node->safe_psql(
	'dbbranch_drop_opfamily_lifecycle_source',
	q[
CREATE OPERATOR FAMILY drop_opfamily_lifecycle_ops USING btree;
CREATE OPERATOR CLASS drop_opfamily_lifecycle_ops
FOR TYPE int USING btree FAMILY drop_opfamily_lifecycle_ops AS STORAGE int;
CREATE TABLE drop_opfamily_lifecycle_rows (
	id int PRIMARY KEY,
	note text NOT NULL
);
INSERT INTO drop_opfamily_lifecycle_rows VALUES (1, 'before-drop');
CHECKPOINT;
DROP OPERATOR FAMILY drop_opfamily_lifecycle_ops USING btree;
INSERT INTO drop_opfamily_lifecycle_rows VALUES (2, 'after-drop');
]);

$stderr = '';
$result = $node->psql(
	'postgres',
	q[CREATE BRANCH dbbranch_drop_opfamily_lifecycle_target FROM DATABASE dbbranch_drop_opfamily_lifecycle_source],
	stderr => \$stderr);
is($result, 0, 'db branch supports dropped operator families');

my $drop_opfamily_lifecycle_rows = $node->safe_psql(
	'dbbranch_drop_opfamily_lifecycle_target',
	q[SELECT string_agg(id || ':' || note, ',' ORDER BY id) FROM drop_opfamily_lifecycle_rows;]);
is($drop_opfamily_lifecycle_rows,
	'1:before-drop,2:after-drop',
	'branch reads rows after operator family drop');
my $drop_opfamily_lifecycle_family = $node->safe_psql(
	'dbbranch_drop_opfamily_lifecycle_target',
	q[
SELECT count(*)
FROM pg_opfamily f
...
JOIN pg_am am ON am.oid = f.opfmethod
WHERE n.nspname = 'public'
  AND f.opfname = 'drop_opfamily_lifecycle_ops'
  AND am.amname = 'btree';
]);
is($drop_opfamily_lifecycle_family, '0',
	'branch does not expose dropped operator family');
my $drop_opfamily_lifecycle_class = $node->safe_psql(
	'dbbranch_drop_opfamily_lifecycle_target',
	q[
SELECT count(*)
FROM pg_opclass c
...
JOIN pg_am am ON am.oid = c.opcmethod
WHERE n.nspname = 'public'
  AND c.opcname = 'drop_opfamily_lifecycle_ops'
  AND am.amname = 'btree';
]);
is($drop_opfamily_lifecycle_class, '0',
	'branch does not expose operator class from dropped family');
